"""Integrating D8 empengae and wing"""
...
#import subcomponent models
from D8_empenage import Empenage
...
class ClimbP(Model):
    ...
    def setup(self, aircraft, state):
        ...
        return self.aircraftP, constraints
...
class Mission(Model):
    ...
    def setup(self, ac):
        ...
        constraints.extend([
            #weight constraints
            TCS([ac['W_{e}'] + ac['W_{payload}'] + ac['numeng'] * ac['W_{engine}'] + ac.wing['W_{struct}'] + ac.empenage.ht['W_{struct}'] + 2*ac.empenage.VT['W_{struct}'] <= Wnofuel]),
        ...
        with SignomialsEnabled():
            constraints.extend([
                cruise['C_{L_h}'] <= 1.1*cruise['C_{L_{ah}}']*cruise['\\alpha_w'],
                cruise['C_{L_h}'] >= 0.9*cruise['C_{L_{ah}}']*cruise['\\alpha_w'],

                # Trim condidtion for each flight segment
                TCS([cruise['x_{ac}']/ac.wing['\\bar{c}_w'] <= cruise['c_{m_{w}}']/cruise['C_{L}'] + xcg/ac.wing['\\bar{c}_w'] + ac.empenage.ht['V_{h}']*(cruise['C_{L_h}']/cruise['C_{L}'])]),
...
##            '\\rho_0': 1.225,
    ...
    mission = Mission(ac)
    m = Model(mission['W_{f_{total}}'], [ac, mission], substitutions)
##    sol = m.localsolve(solver='mosek', verbosity = 4)
    bounds, sol = mission.determine_unbounded_variables(m, solver="mosek",verbosity=4, iteration_limit=100)
