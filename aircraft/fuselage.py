...
class Fuselage(Model):
    ...
    def setup(self):
        ...
        # Variables
        Afloor      = Variable('A_{floor}', 'm^2', 'Floor beam cross sectional area')
        Afuse       = Variable('A_{fuse}', 'm^2', 'Fuselage cross sectional area')
        Ahold       = Variable('A_{hold}', 'm^2', 'Cross sectional area of the hold')
        ...
        Dupswp      = Variable('D_{upsweep}', 'N', 'Form drag due to fuselage upsweep')
#        dRfuse     = Variable('\\Delta R_{fuse}', 'm', 'Offset of lower fuselage cylinder')
        f           = Variable('f', '-', 'Fineness ratio')
        FF          = Variable('FF', '-','Fuselage form factor')
        hfloor      = Variable('h_{floor}', 'm', 'Floor I-beam height') # Value
...
#        Ivshell    = Variable('Ivshell', 'm^4', 'Bending inertia')
        ...
        lcone       = Variable('l_{cone}', 'm', 'Cone length') # added this 
        lnose       = Variable('l_{nose}', 'm', 'Nose length')
        lshell      = Variable('l_{shell}', 'm', 'Shell length')
        Mfloor      = Variable('M_{floor}', 'N*m', 'Maximum bending moment in floor beams')
        nrows       = Variable('n_{rows}', '-', 'Number of rows')
        npass       = Variable('n_{pass}', '-', 'Number of passengers') # Must be multiple of SPR
        Pfloor      = Variable('P_{floor}', 'N', 'Distributed floor load')
        phi         = Variable('\\phi', '-', 'Upsweep angle')
        lamcone     = Variable('\\lambda_{cone}', '-', 'Tailcone radius taper ratio (from xshell2-xtail)')
        plamv       = Variable('p_{\\lambda_v}', '-', '1 + 2*Tail taper ratio')
        qlamv       = Variable('q_{\\lambda_v}', '-', '1 + Tail taper ratio')
        Qv          = Variable('Q_v', 'N*m', 'Torsion moment imparted by vertical tail')
        ...
        sigth       = Variable('\\sigma_{\\theta}', 'N/m^2', 'Hoop stress in skin')
        ...
        Vluggage    = Variable('V_{luggage}', 'm^3', 'Luggage volume')
        ...
        Wluggage    = Variable('W_{luggage}', 'N', 'Passenger luggage weight')
        Wpadd       = Variable('W_{padd}', 'N', 'Misc weights (attendants, food, galley, toilets, doors etc.)')
        Wpass       = Variable('W_{pass}', 'N', 'Passenger weight')
        Wpay        = Variable('W_{pay}', 'N', 'Payload weight') # ~177000N b737.org.uk
...
#        xWwindow   = Variable('xWwindow', 'N*m', 'Mass moment of windows')
        ...
        dh          = Variable('\\Delta h', 1, 'm', 'Required distance from floor to widest part of fuselage')
        dp          = Variable('\\Delta p', 76000-24000, 'Pa', 'Pressure difference across fuselage skin')
        fapu        = Variable('f_{apu}', 0.035, '-', 'APU weight as fraction of payload weight') # [TAS]
        ffadd       = Variable('f_{fadd}', 0.20, '-', 'Skin proportional added weight of local reinforcmenets') # [TAS]
        fframe      = Variable('f_{frame}', 0.25, '-', 'Fractional weight of frame')
        ...
        fpadd       = Variable('f_{padd}', 0.4, '-', 'All other misc weight as fraction of payload weight') # [TAS]
        fseat       = Variable('f_{seat}', 0.10, '-', 'Seat weight as fraction of payload weight')
        fstring     = Variable('f_{string}', 0.35, '-', 'Fractional weight of stringers') # [TAS]
        g           = Variable('g', 9.81, 'm/s^2', 'Gravitational acceleration')
        LF          = Variable('LF', 0.898, '-', 'Load factor')
        Lvmax       = Variable('L_{v_{max}}', 35000, 'N', 'Maximum vertical tail force')
        mu          = Variable('\\mu', 1.4E-5, 'N*s/m^2', 'Dynamic viscosity (35000ft)') # http://www.digitaldutch.com/atmoscalc/
        Nland       = Variable('N_{land}', 6.0,  '-', 'Emergency landing load factor') # [TAS]
        nseat       = Variable('n_{seat}', 186, '-',' Number of seats')
        pcabin      = Variable('p_{cabin}', 75000, 'Pa', 'Cabin air pressure (8000ft)')
        ...
        rhocargo    = Variable('\\rho_{cargo}', 150, 'kg/m^3', 'Cargo density') # boeing 757 freight doc
        ...
        rhoinf      = Variable('\\rho_{\\infty}', 0.38, 'kg/m^3', 'Freestream air density (35,000ft)')
        rholuggage  = Variable('\\rho_{luggage}', 100, 'kg/m^3', 'Luggage density (approximate)')
        rhoskin     = Variable('\\rho_{skin}', 2700, 'kg/m^3', 'Skin density') # [TAS]
        sigfloor    = Variable('\\sigma_{floor}', 30000/0.000145, 'N/m^2', 'Max allowable cap stress') # [TAS]
        ...
        Vinf        = Variable('V_{\\infty}', 234, 'm/s', 'Cruise velocity') # source: Boeing.com
        ...
        Wcarryon    = Variable('W_{carry on}', 15, 'lbf', 'Average carry-on bag weight')
        Wchecked    = Variable('W_{checked}', 40, 'lbf', 'Average checked bag weight')
        Wfix        = Variable('W_{fix}', 3000, 'lbf', 'Fixed weights (pilots, cockpit seats, instrumentation, navcom)')
        ...
        wsys        = Variable('w_{sys}', 0.10, 'm', 'Width between cabin and fuselage skin for systems/frames')
...
#                            Afuse <= np.pi*Rfuse**2 + 2*Rfuse*dRfuse, # Old equation from TASOPT
                            Afuse >= np.pi*Rfuse**2,# + (2./3)*wfloor*dRfuse + (3./2)*hhold**2*dRfuse/wfloor - (3./2)*hhold*dRfuse**2/wfloor, # requires downward pressure on Afuse
    ...
    #                       xWfix == xfix*Wfix,
                            ...
                            lnose >= 5.2*units.m, # TODO: this needs to be driven by something e.g cockpit length
   ...
   #                        xWfloor >= 0.5*(xshell1 + xshell2)*Wfloor,
    ...
    #                        xWcone >= 0.5*(xshell2 + xconend) * Wcone,
                            ...
                            Wluggage >= flugg2*npass*2*Wchecked + flugg1*npass*Wchecked + Wcarryon,
                            Wluggage == Vluggage*g*rholuggage, # luggage is weight constrained
                            Wcargo == Vcargo*g*rhocargo,
                            Vhold >= Vcargo + Vluggage,
                            Vhold == Ahold*lshell,
                            Ahold <= (2./3)*wfloor*hhold + hhold**3/(2*wfloor), # [SP] Harris stocker 1998 (wolfram)
                            dh + hhold + hfloor <= Rfuse,
                            Wpay >= Wpass + Wluggage + Wcargo,
    ...
    #                                 + xWwindow + xWinsul + xWfloor
                            ...
                            # Drag
                            # sources: Raymer, kfid 325 notes
                            lfuse >= lnose+lshell+lcone,
                            f == lfuse/((4/np.pi * Afuse)**0.5), # fineness ratio (Raymer p283)
                            FF >= 1 + 60/f**3 + f/400, # form factor (Raymer p283)
                            Dfrict >= FF * np.pi*Rfuse * mu*Vinf * 0.074*(rhoinf*Vinf*lfuse/mu)**0.8, # kfid p180

                            # Drag due to fuselage upsweep
                            1.13226*phi**1.03759 == Rfuse/lcone, # monomial fit of tan(phi)
                            Dupswp >= 3.83*phi**2.5*Afuse * 0.5*rhoinf*Vinf**2, # Raymer p286
    ...
    def test(self):
        ...
        npt.assert_almost_equal(2*sol('q_{\\lambda_v}'), 1 + sol('p_{\\lambda_v}'), decimal=5)
        npt.assert_almost_equal(sol('A_{hold}'), (2./3)*sol('w_{floor}')*sol('h_{hold}') + sol('h_{hold}')**3/(2*sol('w_{floor}')), decimal=4)
#        npt.assert_almost_equal(sol('\\Delta h') + sol('h_{hold}') + sol('h_{floor}'), sol('R_{fuse}'))
#        npt.assert_almost_equal(sol('A_{fuse}'),  np.pi*sol('R_{fuse}')**2 + 2*sol('R_{fuse}')*sol('\\Delta R_{fuse}'))
#        npt.assert_almost_equal(sol('A_{fuse}'), np.pi*sol('R_{fuse}')**2 + (2./3)*sol('w_{floor}')*sol('\Delta R_{fuse}')
#                                    + (3./2)*sol('h_{hold}')**2*sol('\Delta R_{fuse}')/sol('w_{floor}')
#                                    - (3./2)*sol('h_{hold}')*sol('\Delta R_{fuse}')**2/sol('w_{floor}'))
        npt.assert_almost_equal(sol('W_{buoy}'), (sol('\\rho_{cabin}') - sol('\\rho_{\\infty}'))*sol('g')*sol('V_{cabin}'), decimal=1)
        npt.assert_almost_equal(sol('V_{cone}')*sol('R_{fuse}')*(1 + sol('\\lambda_{cone}')), 2*sol('Q_v')/sol('\\tau_{cone}') * sol('l_{cone}'))
