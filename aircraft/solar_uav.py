...
def solar_uav():
    ...
    rho_solar = Variable("W_{solar}", units="kg/m^2", value=1.2,
    ...
    m_struct = Variable("m_{struct}", units="kg", label="airframe weight")
    wf_struct = Variable("wf_struct", value=0.2,
    ...
    AR = Variable("AR", value=25, label="aspect ratio")
    b = Variable("b", units="ft", label="span")

    # operations / requirements
    m_pay = Variable("m_{pay}", units="kg", value=3, label="payload weight")
    ...
    return Model(S,
                 [P/S >= 1/eta_p*(1/L15D)*(2/rho)**0.5*(wl*g)**1.5,
                  ...
                  m_batt >= (P + P_non_flight)*t_batt/h_batt,
                  m_struct >= wf_struct*m_tot,
                  wl*S >= m_tot,
                  b**2 == S*AR])
