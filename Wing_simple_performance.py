...
class Mission(Model):
    ...
    def __init__(self, subs = None, **kwargs):
        ...
        constraints.extend([
            ...
            TCS([aircraft['W_{e}'] + aircraft['W_{payload}'] + aircraft['numeng'] * aircraft['W_{engine}'] + aircraft.wing.wb['W_{struct}'] <= crs.cruiseP.aircraftP['W_{end}'][-1]]),
        ...
        # self.cost = W_ftotal
        # Model.__init__(self, W_ftotal + s*units('N'), constraints + aircraft + cls + crs, subs)
        Model.__init__(self, W_ftotal, constraints + aircraft + cls + crs, subs)
