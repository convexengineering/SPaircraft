...
from new_d8_wing_imports import Wing#, WingPerformance
...
##            W_burn == aircraft['numeng'] * self.engine['TSFC'] * \
##            thours * self.engine['F'],
...
class ClimbP(Model): # Climb performance constraints
    ...
    def setup(self, aircraft, state, **kwargs):
        ...
        self.engine = self.aircraft.engine
    ...
    def setup(self, **kwargs):
        ...
        constraints.extend([
           ...
           #compute fuel burn from TSFC
            cruise.cruiseP['W_{burn}'] == aircraft['numeng']*aircraft.engine['TSFC'][Nclimb:] * cruise['thr'] * aircraft.engine['F'][Nclimb:],              
            climb.climbP.aircraftP['W_{burn}'] == aircraft['numeng']*aircraft.engine['TSFC'][:Nclimb] * climb['thr'] * aircraft.engine['F'][:Nclimb],

            # Thrust constraint
##            climb.climbP.engine['F'] <= 2 * max(cruise.cruiseP.engine['F']),
...
##            cruise.cruiseP.engine['TSFC'] == .5 * units('1/hr'),
