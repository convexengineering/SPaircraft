...
class D8P(Model):
    ...
    def __init__(self, aircraft, state, **kwargs):
        ...
        self.Pmodels = [self.wingP, self.fuseP,
                        self.engineP, self.VTP]  # , self.HTP]
        ...
        constraints.extend([
            ...
            # compute the drag
            # + self.HTP['D_{ht}']
            TCS([D >= self.wingP['D_{wing}'] + \
                 self.fuseP['D_{fuse}'] + self.VTP['D_{vt}']]),
...
class Mission(Model):
    ...
    def __init__(self, ac, substitutions=None, **kwargs):
        ...
        constraints.extend([
            # weight constraints
            TCS([ac['W_{e}'] + ac['W_{payload}'] + ac['numeng'] * ac['W_{engine}'] + \
                 ac.wing['W_{struct}'] + ac.VT['W_{struct}'] <= W_dry]),  # + ac.HT['W_{struct}']
        ...
        Model.__init__(self, W_ftotal, constraints +
                       ac + climb + cruise, substitutions)
