...
class HorizontalTailNoStruct(Model):
    ...
    def setup(self):
        ...
        amax    = Variable('\\alpha_{max,ht}', '-', 'Max angle of attack, htail')
        ...
        with SignomialsEnabled():
            ...
            constraints.extend([
                ...
                2*q >= 1 + p,
                ymac == (bht/3)*q/p,
                SignomialEquality((2./3)*(1 + taper + taper**2)*croot/q,chma),
                taper == ctip/croot,
...
##                                            * (self.HT[self.HT['\\tau_{ht}']]*100)**0.104391
##                                            * (state['M'])**-0.0177484
##                                + 0.273439 * (Rec/1000)**-0.0017356
##                                           * (self.HT[self.HT['\\tau_{ht}']]*100)**-0.164667
##                                           * (state['M'])**0.0233832
##                                + 0.000150403 * (Rec/1000)**-0.186771
##                                              * (self.HT[self.HT['\\tau_{ht}']]*100)**1.52706
##                                              * (state['M'])**3.89794
##                                + 0.27215 * (Rec/1000)**-0.00170564
##                                          * (self.HT[self.HT['\\tau_{ht}']]*100)**-0.175197
##                                          * (state['M'])**0.0242146
##                                + 0.0608952 * (Rec/1000)**-0.00195729
##                                            * (self.HT[self.HT['\\tau_{ht}']]*100)**0.227082
##                                            * (state['M'])**-0.0439115,
...
class HorizontalTail(Model):
    ...
    def setup(self):
        ...
        with SignomialsEnabled():
            constraints.append([
                self.wb['L_{h_{rect}}'] >= self.HTns['L_{ht_{max}}']/2.
                                          *self.HTns['c_{tip_{ht}}']
                                          *self.HTns['b_{ht}']
                                          /self.HTns['S_{ht}'],
                self.wb['L_{h_{tri}}'] >= self.HTns['L_{ht_{max}}']/4.
                                         *(1-self.wb['taper'])
