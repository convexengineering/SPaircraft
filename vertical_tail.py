...
class VerticalTail(Model):
    ...
    def setup(self, **kwargs):
        ...
        WVT = Variable('W_{VT_system}', 'lbf', 'Total VT System Weight')
...
##                CDvis**0.119892 >= 0.0693931 * (Rec/1000)**-0.0021665 * (self.vt[self.vt['\\tau_{vt}']]*100)**0.104391 * (state['M'])**-0.0177484
##                            + 0.273439 * (Rec/1000)**-0.0017356 * (self.vt[self.vt['\\tau_{vt}']]*100)**-0.164667 * (state['M'])**0.0233832
##                            + 0.000150403 * (Rec/1000)**-0.186771 * (self.vt[self.vt['\\tau_{vt}']]*100)**1.52706 * (state['M'])**3.89794
##                            + 0.27215 * (Rec/1000)**-0.00170564 * (self.vt[self.vt['\\tau_{vt}']]*100)**-0.175197 * (state['M'])**0.0242146
##                            + 0.0608952 * (Rec/1000)**-0.00195729 * (self.vt[self.vt['\\tau_{vt}']]*100)**0.227082 * (state['M'])**-0.0439115,
                    CDvis**1.18909 >= 2.43701e-77 * (Rec)**-0.52841 * (self.vt['\\tau_{vt}'])**133.796 * (state['M'])**1022.7
                        ...
                        #Martin's NACA fit
##            CDvis**1.5846 >= 0.000195006 * (Rec)**-0.508965 * (self.vt[self.vt['\\tau_{vt}']]*100)**1.62106 * (state['M'])**0.670788
##                        + 9.25066e+18 * (Rec)**-0.544817 * (self.vt[self.vt['\\tau_{vt}']]*100)**1.94003 * (state['M'])**240.136
##                        + 2.23515e-05 * (Rec)**0.223161 * (self.vt[self.vt['\\tau_{vt}']]*100)**0.0338899 * (state['M'])**0.0210705,

            #Philippe thesis fit
            # Vertical tail viscous drag in cruise
            # Data fit from Xfoil
##            CDvis**0.125 >= 0.19*(self.vt[self.vt['\\tau_{vt}']])**0.0075 *(Rec)**0.0017
##                        + 1.83e+04*(self.vt[self.vt['\\tau_{vt}']])**3.54*(Rec)**-0.494
##                        + 0.118*(self.vt[self.vt['\\tau_{vt}']])**0.0082 *(Rec)**0.00165
##                        + 0.198*(self.vt[self.vt['\\tau_{vt}']])**0.00774*(Rec)**0.00168,
            ])
        else:
            None
